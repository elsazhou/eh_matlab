...
%Q_all=1*10^(-1).*[0.25 2;0.5 2;0.75 1;1 1;1.5 1;2 1;2.5 1;3 1;4 1];%x1 is changing
eta=0.8;
m=size(Q_all,1);
d=[1;2;1];
sigma=[10^(-4);10^(-4)];
...
tot_iter=0;
step=0.03;
load('ed_maxthrpt_q2c_ca.mat');
load('ed_maxthrpt_q2c_cb.mat');

for ic=1:m
    Q=Q_all(ic,:)';
    for t04=opt1_time4(ic)-opt1_time3(ic)/3:step:(opt1_time4(ic)+opt1_time3(ic)/3-step/3)
        for t01=opt1_time1(ic)-opt1_time3(ic)/3:step:(opt1_time1(ic)+opt1_time3(ic)/3-step/3)
            for t02=opt1_time2(ic)-opt1_time3(ic)/3:step:(opt1_time2(ic)+opt1_time3(ic)/3-step/3)
                t03=1-t01-t02-t04;
                t0=[t01;t02;t03;t04];
                ...
                if(maxthrpt1(ic)<sumthrp1)
                    ...
                    save ed_maxthrpt_q2c_ca.mat ic Q_all opt1_time1 opt1_time2 opt1_time3 opt1_time4 opt1_t0 maxthrpt1 opt1_rho
                end
                tot_iter=tot_iter+1;
            end
        end
...
for ic=1:m
    Q=Q_all(ic,:)';
    for t04=opt2_time4(ic)-opt2_time3(ic)/3:step:(opt2_time4(ic)+opt2_time3(ic)/3-step/3)
        for t01=opt2_time1(ic)-opt2_time3(ic)/3:step:(opt2_time1(ic)+opt2_time3(ic)/3-step/3)
            for t02=opt2_time2(ic)-opt2_time3(ic)/3:step:(opt2_time2(ic)+opt2_time3(ic)/3-step/3)
                t03=1-t01-t02-t04;
                t0=[t01;t02;t03;t04];
                ...
                if(maxthrpt2(ic)<sumthrp2)
                    ...
                    save ed_maxthrpt_q2c_cb.mat ic Q_all opt2_time1 opt2_time2 opt2_time3 opt2_time4 opt2_t0 maxthrpt2 opt2_rho
                end
                tot_iter=tot_iter+1;
            end
        end
...
title('Comparison of Sum-Throughput (X1=100mW,\eta=0.8)')
