...
%ini t
tot_iter=0;
load('e_maxthrpt_q2c_ca.mat');
load('e_maxthrpt_q2c_cb.mat');
step=0.005;
for ic=10:m
    Q=Q_all(ic,:)';
    for t01=(opt5_time1(ic)-opt5_time3(ic)/2):step:(opt5_time1(ic)+opt5_time3(ic)/2-0.01)
        for t02=(opt5_time2(ic)-opt5_time3(ic)/2):step:(opt5_time2(ic)+opt5_time3(ic)/2-0.01)
            t03=1-t01-t02;
            t0=[t01;t02;t03];
            ...
            if(maxthrpt5(ic)<sumthrp1)
                ...
                save e_maxthrpt_q2c_ca.mat ic Q_all opt5_time1 opt5_time2 opt5_time3 opt5_t0 maxthrpt5 step
            end
            tot_iter=tot_iter+1;
        end
    end
...
for ic=1:m
    ...
    for t01=(opt6_time1(ic)-opt6_time3(ic)/2):step:(opt6_time1(ic)+opt6_time3(ic)/2-0.01)
        for t02=(opt6_time2(ic)-opt6_time3(ic)/2):step:(opt6_time2(ic)+opt6_time3(ic)/2-0.01)
            t03=1-t01-t02;
            t0=[t01;t02;t03];
            ...
            if(maxthrpt6(ic)<sumthrp2)
                ...
                save e_maxthrpt_q2c_cb.mat ic Q_all opt6_time1 opt6_time2 opt6_time3 opt6_t0 maxthrpt6 step
            end
            tot_iter=tot_iter+1;
        end
    end
