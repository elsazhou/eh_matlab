...
opt8_p2=zeros(m,1);
%ini t
load('p1_nc_q1c_ca_p0.mat');
load('p1_nc_q1c_cb_p0.mat');
tot_iter=0;
for ic=1:m
    Q=Q_all(ic,:)'; 
    p1s=2*opt7_p0(1,ic)/3;p1e=4*opt7_p0(1,ic)/3;
    p2s=2*opt7_p0(2,ic)/3;p2e=4*opt7_p0(2,ic)/3;
   for p01=p1s:(p1e-p1s)/20:p1e
        for p02=p2s:(p2e-p2s)/20:p2e
            p0=[p01;p02];
            ...
            if(maxthrpt7(ic)<sumthrp1)
                ...
                opt7_p1(ic)=Pka1(1);
                opt7_p2(ic)=Pka1(2);
                save p1_nc_q1c_ca_p0.mat ic Q_all opt7_time1 opt7_time2 opt7_time3 opt7_p0 maxthrpt7 opt7_p1 opt7_p2
            end
            tot_iter=tot_iter+1;
        end
    end
...
for ic=1:m
    Q=Q_all(ic,:)';
    p1s=2*opt8_p0(1,ic)/3;p1e=4*opt8_p0(1,ic)/3;
    p2s=2*opt8_p0(2,ic)/3;p2e=4*opt8_p0(2,ic)/3;
    for p01=p1s:(p1e-p1s)/20:p1e
        for p02=p2s:(p2e-p2s)/20:p2e
            p0=[p01;p02];
            ...
            if(maxthrpt8(ic)<sumthrp2)
                ...
                opt8_p1(ic)=Pkb1(1);
                opt8_p2(ic)=Pkb1(2);              
                save p1_nc_q1c_cb_p0.mat ic Q_all opt8_time1 opt8_time2 opt8_time3 opt8_p0 maxthrpt8 opt8_p1 opt8_p2
            end
            tot_iter=tot_iter+1;
        end
    end
