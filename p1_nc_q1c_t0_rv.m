...
for ic=1:m
    Q=Q_all(ic,:)';
    t1s=2*opt7_t0(1,ic)/3;t1e=4*opt7_t0(1,ic)/3;
    t2s=2*opt7_t0(2,ic)/3;%t2e=10*opt7_t0(2,ic)/9;
   for t01=t1s:(t1e-t1s)/20:t1e
        for t02=t2s:(0.95-t01-t2s)/20:(0.95-t01)
            t03=1-t01-t02;
            t0=[t01;t02;t03];
...
for ic=1:m
    Q=Q_all(ic,:)';
    t1s=2*opt8_t0(1,ic)/3;t1e=4*opt8_t0(1,ic)/3;
    t2s=2*opt8_t0(2,ic)/3;%t2e=10*opt8_t0(2,ic)/9;
   for t01=t1s:(t1e-t1s)/20:t1e
        for t02=t2s:(0.95-t01-t2s)/20:(0.95-t01)
            t03=1-t01-t02;
            t0=[t01;t02;t03];
...
title('Comparison of Sum-Throughput (X2=100mW,ini t)')
